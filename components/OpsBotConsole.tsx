...
export function OpsBotConsole() {
  ...
  return (
    <main className="mx-auto flex min-h-svh w-full max-w-6xl flex-col gap-8 bg-background px-4 py-4 text-foreground sm:px-6 sm:py-8">
      ...
      <section
        className="grid grid-cols-1 gap-4 md:grid-cols-[minmax(320px,0.9fr)_minmax(0,1.1fr)]"
      ...
      >
        <Card>
          <CardHeader>
            ...
            <CardTitle className="text-xl leading-[26px] tracking-normal">Ask OpsBot</CardTitle>
          </CardHeader>
          ...
          <CardContent>
            ...
            <div
              className="mt-4 rounded-md border border-border bg-secondary p-4"
              aria-live="polite"
            >
        ...
        <Card>
          <CardHeader>
            ...
            <CardTitle className="text-xl leading-[26px] tracking-normal">
      ...
      <Card aria-label="Recent mocked actions">
        <CardHeader>
          <p className="text-xs font-medium uppercase leading-4 text-muted-foreground">
            Local Log
          </p>
          <CardTitle className="text-xl leading-[26px] tracking-normal">Recent Actions</CardTitle>
        </CardHeader>
        <CardContent>
        {actionLog.length === 0 ? (
          <p className="text-muted-foreground">
            No actions yet. Tap a visitor request to create one.
          </p>
        ) : (
          <ol className="grid gap-2">
            {actionLog.map((item) => (
              <li
                className="grid grid-cols-1 gap-2 rounded-md border border-border p-3 sm:grid-cols-[minmax(0,1fr)_auto_auto] sm:items-center"
                key={`${item.intent}-${item.at}`}
              >
                <span className="font-medium">{item.label}</span>
                <code className="w-fit rounded-md border border-border bg-secondary px-1.5 py-1 font-mono text-[13px] leading-[18px] text-foreground">
                  {item.robot_action}
                </code>
                <time className="font-mono text-xs leading-4 text-muted-foreground">{item.at}</time>
              </li>
            ))}
          </ol>
        )}
